...
if (empty($_POST['firstname'])
   ...
   || empty($_POST['password'])
   	...
   	exit(header('Location: ../signup.php?signup=empty'));
}
...
if (mysqli_num_rows($valid_username) > 0)
	exit(header('Location: ../signup.php?signup=username'));
...
if(mysqli_num_rows($valid_email) > 0)
	exit(header('Location: ../signup.php?signup=email'));
...
if($insert === TRUE){
	...
	exit(header('Location: ../signup.php?signup=insert'));
}
